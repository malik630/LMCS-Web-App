...
class DashboardProjetDetailView extends View
{
    ...
    public function render()
    {
        ...
        // Membres
        $this->renderMembresSection($membres, $isResponsable, $projet, $availableMembers);
    ...
    private function renderMembresSection($membres, $isResponsable, $projet, $availableMembers)
    {
        Section::create('Membres du projet (' . count($membres) . ')', function() use ($membres, $isResponsable, $projet, $availableMembers) {
            if (empty($membres)) {
            ...
            if ($isResponsable) {
                $this->renderAddMembreForm($projet, $availableMembers);
            }
        }, 'bg-white');
    }
...
<?php
    ...
    private function renderAddMembreForm($projet, $availableMembers)
    {
        if (empty($availableMembers)) {
            ?>
<div class="mt-6 p-4 bg-gray-50 rounded-lg">
    <p class="text-sm text-gray-600 text-center">Tous les membres disponibles ont déjà été ajoutés au projet.</p>
</div>
<?php
            return;
        }
